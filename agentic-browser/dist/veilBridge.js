...
export class VeilBridge {
    page;
    serverUrl;
    constructor(page, serverUrl = "http://localhost:3001") {
        this.page = page;
                ...
                const extractFallback = () => {
                    ...
                    document.querySelectorAll("input, button, a, select, textarea, [role]").forEach((el, idx) => {
                        ...
                        elements.push({
                            ...
                            role,
                            label: el.innerText || el.getAttribute("aria-label") || el.getAttribute("placeholder") || el.getAttribute("name") || "",
     ...
     */
    async requestPlan(userGoal, pageMap) {
    ...
    async executeAction(action) {
        ...
        try {
            if (action.type === "highlight") {
                await this.page.evaluate((tgt) => {
                    const el = tgt.elementId ? document.querySelector(`[data-veil-id="${tgt.elementId}"]`) ||
                        ...
                        Array.from(document.querySelectorAll("button, a, input")).find(e => e.innerText?.includes(tgt.label || "")) : null;
                    if (el) {
                        el.style.outline = "3px solid #00f0ff";
                        el.style.boxShadow = "0 0 15px rgba(0, 240, 255, 0.6)";
            ...
            if (action.type === "type" || action.type === "fill") {
                ...
                await this.page.evaluate(({ tgt, val }) => {
                    const el = (tgt.selector ? document.querySelector(tgt.selector) : null) ||
                        Array.from(document.querySelectorAll("input, textarea")).find(e => e.getAttribute("placeholder")?.toLowerCase().includes((tgt.label || "").toLowerCase()) ||
                            e.getAttribute("name")?.toLowerCase().includes((tgt.label || "").toLowerCase()) ||
                            e.id.toLowerCase().includes((tgt.label || "").toLowerCase()));
            ...
            if (action.type === "click") {
                await this.page.evaluate((tgt) => {
                    const el = (tgt.selector ? document.querySelector(tgt.selector) : null) ||
                        Array.from(document.querySelectorAll("button, a, input[type='submit']")).find(e => e.innerText?.toLowerCase().includes((tgt.label || "").toLowerCase()) ||
                            e.id.toLowerCase().includes((tgt.label || "").toLowerCase()));
     ...
     */
    async runGoal(userGoal, options = {}) {
