...
function matchesAnyPrefixOrKeyword(text, keywords) {
...
function extractIntentFromGoal(goal) {
    const raw = goal.trim();
...
export function classifyGoal(userGoal, pageMap) {
...
export function isInformationalMode(mode) {
    return mode === "information" || mode === "informational";
}
export function requiresBrowserAction(mode) {
...
export function shouldExecuteActions(classification) {
    if (classification.requiresClarification)
        return false;
    if (isInformationalMode(classification.mode))
        return false;
    if (classification.mode === "ambiguous" || classification.mode === "unsupported")
        return false;
    return true;
}
