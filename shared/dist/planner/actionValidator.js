...
export function findMatchingPageElement(target, pageElements) {
    ...
    if (target.elementId) {
        const byDomId = pageElements.find((el) => el.elementId && el.elementId.toLowerCase() === target.elementId.toLowerCase());
        if (byDomId)
            return byDomId;
    }
    ...
    if (target.bounds && typeof target.bounds.x === "number") {
        ...
        const byBounds = pageElements.find((el) => {
            if (!el.bounds)
                return false;
            return (el.bounds.x < (bounds.x ?? 0) + (bounds.width ?? 0) &&
...
function isActionRelevantToGoal(action, classification, pageElements) {
...
function isDestructiveAction(action, targetElement) {
...
export function validateActionAgainstGoal(action, context) {
    ...
    const duplicateAction = previousActions.find((prev) => prev.type === action.type && prev.target?.elementId === action.target?.elementId);
    if (duplicateAction) {
        return {
            valid: false,
...
export function validatePlanActions(planActions, context) {
    return planActions.map((action, index) => validateActionAgainstGoal(action, { ...context, stepNumber: context.stepNumber + index }));
}
