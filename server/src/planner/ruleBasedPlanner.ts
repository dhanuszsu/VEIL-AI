...
import { classifyGoal, GoalClassification, GoalMode, shouldExecuteActions } from "@privatesight/shared";
...
function extractKeywords(target: string): string[] {
...
function findElementByLabelOrRole(pageMap: PageMap, keywords: string[]): SanitizedElement | undefined {
  ...
  const scoredElements = pageMap.elements
    ...
    .map(el => {
      ...
      for (const kw of lowerKeywords) {
        ...
        if (labelLower === kw) { score += 100; kwMatched = true; }
        else if (labelLower.includes(kw)) { score += 50; kwMatched = true; }
        
        if (textLower.includes(kw)) { score += 30; kwMatched = true; }
        if (placeholderLower.includes(kw)) { score += 40; kwMatched = true; }
...
function findElementByRoleAndLabel(pageMap: PageMap, roles: string[], keywords: string[]): SanitizedElement | undefined {
  const lowerKeywords = keywords.map((k) => k.toLowerCase());
...
function findSubmitLikeElement(pageMap: PageMap): SanitizedElement | undefined {
  ...
  return findElementsByRole(pageMap, ["button"]).find(
...
function findSearchElement(pageMap: PageMap): SanitizedElement | undefined {
  ...
  return findElementsByRole(pageMap, ["searchbox", "textbox"]).find((el) =>
...
function findElementByNameOrId(pageMap: PageMap, keywords: string[]): SanitizedElement | undefined {
  const lowerKeywords = keywords.map((k) => k.toLowerCase());
  return pageMap.elements.find(
    (el) =>
      el.visible &&
...
function findClickableElements(pageMap: PageMap): SanitizedElement[] {
  return findElementsByRole(pageMap, ["button", "link", "menuitem", "tab"]);
}

function createAction(
  type: ActionType,
  target: SanitizedElement | { elementId: string; id?: string; selector?: string; label?: string; bounds?: any } | undefined,
  reason: string,
  confidence: number,
  extra?: Partial<ServerAction>
): ServerAction | null {
  ...
  const text = "textContent" in target ? (target as any).textContent || target.label : label;
...
function planForInformational(
  ...
  if (interactiveCount > 0) capabilities.push(`interact with ${interactiveCount} button(s)/link(s)`);
  if (formFieldCount > 0) capabilities.push(`fill ${formFieldCount} form field(s)`);
  if (hasSearch) capabilities.push("search");
  if (hasSubmit) capabilities.push("submit forms");
...
function planForFindOrHighlight(
  classification: GoalClassification,
  pageMap: PageMap
): { actions: ServerAction[]; summary: string; confidence: number } {
...
function planForSubmit(
  _classification: GoalClassification,
  pageMap: PageMap
): { actions: ServerAction[]; summary: string; confidence: number; requiresConfirmation: boolean } {
